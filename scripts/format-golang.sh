...
echo "${GOFILES}" | xargs -I {} sh -c 'goimports -v -w -e -local github.com/kainwinterheart {}'

echo "formatting with gci.."
echo "${GOFILES}" |
  xargs -I {} sh -c 'gci write --skip-generated -s standard -s default -s "Prefix(github.com/kainwinterheart)" {}'
