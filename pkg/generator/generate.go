...
import (
	...
	"github.com/kainwinterheart/go-jsonschema/internal/x/text"
...
func New(config Config) (*Generator, error) {
	formatters := []formatter{
		&jsonFormatter{config: &config},
	}
	if config.ExtraImports {
		formatters = append(formatters, &yamlFormatter{})
	}

	// Set default resolve extensions if not provided
	if config.ResolveExtensions == nil {
		config.ResolveExtensions = []string{".json"}
	}
