// Code generated by github.com/kainwinterheart/go-jsonschema, DO NOT EDIT.

package test

import "encoding/json"
import "fmt"
import "github.com/kainwinterheart/go-jsonschema/pkg/types"
import yaml "gopkg.in/yaml.v3"

type Date struct {
	// myobject corresponds to the JSON schema field "myObject".
	myobject *Datemyobject `json:"myObject,omitempty,omitzero" yaml:"myObject,omitempty" mapstructure:"myObject,omitempty"`
}

type DateBuilder struct {
	myobject *Datemyobject
}

func (b *DateBuilder) Build() *Date {
	return &Date{
		myobject: b.myobject,
	}
}

func (b *DateBuilder) WithMyObject(v *Datemyobject) *DateBuilder {
	b.myobject = v
	return b
}

func (o *Date) Clone() *DateBuilder {
	return NewDateBuilder(o)
}

func (o *Date) MyObject() *Datemyobject {
	return o.myobject
}

// UnmarshalYAML implements yaml.Unmarshaler.
func (j *Date) UnmarshalYAML(value *yaml.Node) error {
	type PlainRaw struct {
		Myobject *Datemyobject
	}
	type Plain Date
	var rawStruct PlainRaw
	if err := value.Decode(&rawStruct); err != nil {
		return err
	}
	var plain Plain
	plain.myobject = rawStruct.Myobject
	plain = plain
	*j = Date(plain)
	return nil
}

// UnmarshalJSON implements json.Unmarshaler.
func (j *Date) UnmarshalJSON(value []byte) error {
	type DateHelper struct {
		Myobject *Datemyobject `json:"myObject,omitempty,omitzero"`
	}
	type Plain Date
	var helper DateHelper
	if err := json.Unmarshal(value, &helper); err != nil {
		return err
	}
	var plain Plain
	plain.myobject = helper.Myobject
	*j = Date(plain)
	return nil
}

// MarshalJSON implements json.Marshaler.
func (j *Date) MarshalJSON() ([]byte, error) {
	type DateMarshalHelper struct {
		Myobject *Datemyobject `json:"myObject,omitempty,omitzero"`
	}
	helper := DateMarshalHelper{
		Myobject: j.myobject,
	}
	return json.Marshal(helper)
}

type Datemyobject struct {
	// mydate corresponds to the JSON schema field "myDate".
	mydate types.SerializableDate `json:"myDate" yaml:"myDate" mapstructure:"myDate"`
}

type DatemyobjectBuilder struct {
	mydate types.SerializableDate
}

func (b *DatemyobjectBuilder) Build() *Datemyobject {
	return &Datemyobject{
		mydate: b.mydate,
	}
}

func (b *DatemyobjectBuilder) WithMyDate(v types.SerializableDate) *DatemyobjectBuilder {
	b.mydate = v
	return b
}

func (o *Datemyobject) Clone() *DatemyobjectBuilder {
	return NewDatemyobjectBuilder(o)
}

func (o *Datemyobject) MyDate() types.SerializableDate {
	return o.mydate
}

// UnmarshalYAML implements yaml.Unmarshaler.
func (j *Datemyobject) UnmarshalYAML(value *yaml.Node) error {
	var raw map[string]interface{}
	if err := value.Decode(&raw); err != nil {
		return err
	}
	if _, ok := raw["myDate"]; raw != nil && !ok {
		return fmt.Errorf("field myDate in Datemyobject: required")
	}
	type PlainRaw struct {
		Mydate types.SerializableDate
	}
	type Plain Datemyobject
	var rawStruct PlainRaw
	if err := value.Decode(&rawStruct); err != nil {
		return err
	}
	var plain Plain
	plain.mydate = (types.SerializableDate)(rawStruct.Mydate)
	plain = plain
	*j = Datemyobject(plain)
	return nil
}

// UnmarshalJSON implements json.Unmarshaler.
func (j *Datemyobject) UnmarshalJSON(value []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(value, &raw); err != nil {
		return err
	}
	if _, ok := raw["myDate"]; raw != nil && !ok {
		return fmt.Errorf("field myDate in Datemyobject: required")
	}
	type DatemyobjectHelper struct {
		Mydate types.SerializableDate `json:"myDate"`
	}
	type Plain Datemyobject
	var helper DatemyobjectHelper
	if err := json.Unmarshal(value, &helper); err != nil {
		return err
	}
	var plain Plain
	plain.mydate = (types.SerializableDate)(helper.Mydate)
	*j = Datemyobject(plain)
	return nil
}

// MarshalJSON implements json.Marshaler.
func (j *Datemyobject) MarshalJSON() ([]byte, error) {
	type DatemyobjectMarshalHelper struct {
		Mydate types.SerializableDate `json:"myDate"`
	}
	helper := DatemyobjectMarshalHelper{
		Mydate: j.mydate,
	}
	return json.Marshal(helper)
}

func NewDateBuilder(o *Date) *DateBuilder {
	if o == nil {
		return &DateBuilder{}
	}
	return &DateBuilder{
		myobject: o.myobject,
	}
}

func NewDatemyobjectBuilder(o *Datemyobject) *DatemyobjectBuilder {
	if o == nil {
		return &DatemyobjectBuilder{}
	}
	return &DatemyobjectBuilder{
		mydate: o.mydate,
	}
}
